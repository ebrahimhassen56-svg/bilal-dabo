...
def load_expenses():
    conn = sqlite3.connect(DB_FILE)
    df = pd.read_sql_query("SELECT date, item, amount FROM expenses", conn)
    conn.close()
...
if check_password():
    ...
    # --- 📜 [4] ሪፖርት ---
        ...
        if all_s:
            ...
            staff_recs = sorted([r for r in staff_history.values() if r.get('staff_name') == sel_staff], key=lambda x: x.get('date', ''), reverse=True)
            ...
            st.subheader(f"📅 የ {sel_staff} የዱቤ ዝርዝር መረጃ")
    ...
    # --- 🛠️ [5] ማስተካከያ (EDIT) ---
    elif choice == "🛠️ [5] ማስተካከያ (EDIT)":
        st.header("🛠️ ማስተካከያ (EDIT) ማዕከል")
        ...
        if opt_main.startswith("[3]"):
            ...
            if s_name and not matches: st.error("❌ ሰራተኛው አልተገኘም!")
                ...
                sel_day = st.selectbox("ቀን መምረጫ", range(len(matches)), format_func=lambda x: f"ቀን: {matches[x][1]['date']} | ወጣ: {matches[x][1].get('morning_load',0)} | ገባ: {matches[x][1].get('returned',0)}")
    ...
    # --- 💸 [6] ወጪ መመዝገቢያ ---
        ...
        with col2:
            ...
            if expenses_data.get("list"):
                st.dataframe(pd.DataFrame(expenses_data["list"]), use_container_width=True)
